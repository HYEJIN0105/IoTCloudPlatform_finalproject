...
void getDeviceStatus(char* payload) {
  // Read temperature, humidity as Celsius (the default)
  ...
  sprintf(payload,"{\"state\":{\"reported\":{\"temperature\":\"%0.2f\",\"humidity\":\"%0.2f\",\"LED\":\"%s\",\"BUZZER\":\"%s\"}}}",t,h,led,buzzer);
}
...
void onMessageReceived(int messageSize) {
  ...
  if (strcmp(led,"ON")==0) {
    if(strcmp(buzzer,"ON")==0){
    led1.on();
    buzzer1.on();
    sprintf(payload,"{\"state\":{\"reported\":{\"LED\":\"%s\",\"BUZZER\":\"%s\"}}}","ON","ON");
    sendMessage(payload);}
    
    else if(strcmp(buzzer,"OFF")==0){
    led1.on();
    buzzer1.off();
    sprintf(payload,"{\"state\":{\"reported\":{\"LED\":\"%s\",\"BUZZER\":\"%s\"}}}","ON","OFF");
    sendMessage(payload);}
  } 
    ...
    led1.off();
    buzzer1.on();
    sprintf(payload,"{\"state\":{\"reported\":{\"LED\":\"%s\",\"BUZZER\":\"%s\"}}}","OFF","ON");
    sendMessage(payload);}
